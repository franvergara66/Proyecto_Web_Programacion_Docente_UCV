...
function xhr2CompletadoAnosLectivos2(){
	
	
	if (xhr21.readyState != 4) 
		return;
	anos_lectivos = new Array();
	anos_lectivos=xhr21.responseText.split('\n');
	
	
	
}

function xhr2CompletadoCodigos2() {
	
	if (xhr12.readyState != 4) 
		return;
	
	codes = new Array();
	codes=xhr12.responseText.split('\n');
	
}

function xhr2Completado2() {
			
	if (xhr11.readyState != 4) 
		return;
	
	asignatures = new Array();
	asignatures=xhr11.responseText.split('\n');
...
<center>
	...
	<div id="centro" style="left:105">
		...
		<div id="centro_der" name="centro_der" >
			<form id="buscador" name="buscador" action="buscarPD" onsubmit="return procesarDatos(this);">
			...
			<div id="ac4update" style="display:none;border:1px solid black;background-color:dddddd; text-align:left; font-family:Tahoma; font-weight:regular; font-size:14px; color:#403f81; top:200; left:150; position:absolute"></div>
			<form id="programacion_docente" name="programacion_docente" onsubmit="return revisarTabla(this);" method="get" action="modificarMaterias">
				...
				<%	if(session.getAttribute("borrar")!=null){
					...
					out.print("<b>Periodo Academico " + session.getAttribute("ano").toString());
					out.println(" - " + session.getAttribute("periodo").toString() + "</b>");
			...
			<%ArrayList error=((ArrayList)session.getAttribute("PD_resultados"));
			  ArrayList errorPD=((ArrayList)session.getAttribute("PD_resultados_error"));
					if (error!=null ){
						System.out.println("loooooooooooooooooooonnnnnnnnggggggggggiiiiiiiiiittttttttttuuuuuuuuuuuuuuuudddd!!!" + error.size());
						if(error.size()>0){
				%>
  ...
  <script>
	...
	if (document.cookie.length > 0){
		...
		/*****************************************************************************************************************************************/
			...
			try {
			// Internet Explorer 5.5+
			xhr11 = new ActiveXObject("Msxml2.XMLHTTP");
			} catch (e) {
				try {
				// Internet Explorer 5.0+
			...
			if (!xhr11 && typeof XMLHttpRequest != 'undefined') {
				// Mozilla 1.0+ y Safari 1.2+
				xhr11 = new XMLHttpRequest();
			}
		
			xhr11.onreadystatechange=xhr2Completado2;
			xhr11.open("GET", "materias.txt", true);
			xhr11.send(null);
			//alert(asignatures.length);
	/*****************************************************************************************************************************************/
			...
			try {
				// Internet Explorer 5.5+
				xhr12 = new ActiveXObject("Msxml2.XMLHTTP");
			} catch (e) {
				try {
					// Internet Explorer 5.0+
			...
			if (!xhr12 && typeof XMLHttpRequest != 'undefined') {
				// Mozilla 1.0+ y Safari 1.2+
				xhr12 = new XMLHttpRequest();
			}
			xhr12.onreadystatechange=xhr2CompletadoCodigos2;
			xhr12.open("GET", "codigos.txt", true);
			xhr12.send(null);
			
	/*****************************************************************************************************************************************/		
			try {
			// Internet Explorer 5.5+
			xhr21 = new ActiveXObject("Msxml2.XMLHTTP");
			} catch (e) {
				try {
				// Internet Explorer 5.0+
					xhr21 = new ActiveXObject("Microsoft.XMLHTTP");
				} catch (ex) {
				xhr21 = false;
				}
			}
			...
			if (!xhr21 && typeof XMLHttpRequest != 'undefined') {
				// Mozilla 1.0+ y Safari 1.2+
				xhr21 = new XMLHttpRequest();
			}
		
			xhr21.onreadystatechange=xhr2CompletadoAnosLectivos2;
			xhr21.open("GET", "anos_lectivos.txt", true);
			xhr21.send(null);
			//alert("pac x aki");
			//alert("2do aviso hay "+ anos_lectivos.length+"años lectivos");
